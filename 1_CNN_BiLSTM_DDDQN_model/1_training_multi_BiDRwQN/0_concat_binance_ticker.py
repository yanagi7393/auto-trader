...
concat_phase = 'train' # train or test

#make dir
if os.path.isdir("./csv_{}".format(concat_phase)):
    pass
else:
    os.makedirs("./csv_{}".format(concat_phase))

#dir of ticker data
dir_ = '../0_get_ticker/'
file_dir_list = [dir_+'btcusdt/', dir_+'ethusdt/', dir_+'eosusdt/', dir_+'neousdt/', dir_+'adausdt/', dir_+'xrpusdt/']
coin_list = ['btcusdt','ethusdt','eosusdt','neousdt','adausdt','xrpusdt']
...
def concatnating_data(dir__, number, file_len):
    for i in range(6):
        ...
        dataframe.to_csv('{}_{}.csv'.format(coin_list[number],i), index=False)
