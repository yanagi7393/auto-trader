...
class stock_state(object):
    ...
    def reset(self,item_state,file_state,start_point):
        ...
        self.now_point = 0
        self.reward = 0
        self.buy_price = self.stock_frame['weigted_price'][0]
        self.sell_price = self.stock_frame['weigted_price'][0]
        self.before_buy_have_money = 100
        self.money = 100
        self.stock = 0
    ...
    def calc_reward(self, buysell): 
        if buysell == 'buy':
            used_money_percent = (self.before_buy_have_money - self.money)/self.total_money
            self.reward = -1 * ((self.buy_price*(1+(self.fee/100)) - self.sell_price)/self.sell_price)*used_money_percent*100
        
        if buysell == 'sell':
            used_stock_percent = ((self.before_sell_have_stock - self.stock)*self.sell_price)/self.total_money
            self.reward = 1 * ((self.sell_price*(1-(self.fee/100)) - self.buy_price)/self.buy_price)*used_stock_percent*100
    ...
    def buy(self, percent):
        if self.money == 0:
            ...
            #save before buy have money
            self.before_buy_have_money = self.money
    ...
    def sell(self, percent):
        if self.stock == 0:
            ...
            #save before sell have stock
            self.before_sell_have_stock = self.stock
