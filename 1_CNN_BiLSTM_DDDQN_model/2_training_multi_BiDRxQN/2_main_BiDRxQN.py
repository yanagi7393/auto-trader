...
max_epLength = 180 #The max length of episode.
anneling_steps = max_epLength*50 #How many steps of training to reduce startE to endE.
num_episodes = 100000 #How many episodes to train network with.
pre_train_steps = max_epLength*50 #How many steps of random actions before training begins.
load_model = True #Whether to load a saved model.
pre_train_switch = 'on' #Whether pre-train or not
...
class Qnetwork():
    def __init__(self,h_size,rnn_cell_fw1,rnn_cell_bw1,rnn_cell_fw2,rnn_cell_bw2,myScope):
        ...
        #split
        self.convFlat1 = layers.fully_connected(self.convFlat, h_size, activation_fn=None)#[batch*multi_count, seq_len, 133]
        self.convFlat2 = layers.fully_connected(self.convFlat, h_size, activation_fn=None)#[batch*multi_count, seq_len, 133]
        ...
        self.rnn1 = tf.concat([self.output_fw1[:,-1,:], self.output_bw1[:,-1,:]], axis=1) #[batch*multi_count, 133*2]
        self.rnn1 = layers.fully_connected(self.rnn1, h_size, activation_fn=None) #[batch*multi_count, 133]

        #2 predict LSTM-----------------------------------------------------------------------------------------------------
        #for training [input: 0~59 -> label: 60]
        (self.output_fw2_tn, self.output_bw2_tn), _ = tf.nn.bidirectional_dynamic_rnn(                                    inputs=self.convFlat2[:,:seq_len-1,:],cell_fw=rnn_cell_fw2,cell_bw=rnn_cell_bw2,                                    dtype=tf.float32,scope=myScope+'_rnn2')
        
        self.rnn2_tn = tf.concat([self.output_fw2_tn[:,-1,:], self.output_bw2_tn[:,-1,:]], axis=1) #[batch*multi_count, 133*2]
        self.rnn2_tn = layers.fully_connected(self.rnn2_tn, h_size, activation_fn=None) #[batch*multi_count, 133]
        
        #for output [input: 1~60 -> predict: 61]
        (self.output_fw2_out, self.output_bw2_out), _ = tf.nn.bidirectional_dynamic_rnn(                                    inputs=self.convFlat2[:,1:,:],cell_fw=rnn_cell_fw2,cell_bw=rnn_cell_bw2,                                    dtype=tf.float32,scope=myScope+'_rnn2')
        
        self.rnn2 = tf.concat([self.output_fw2_out[:,-1,:], self.output_bw2_out[:,-1,:]], axis=1) #[batch*multi_count, 133*2]
        ...
        self.rnn_price = layers.fully_connected(self.rnn_price, h_size//label_len, activation_fn=tf.nn.relu)
        self.rnn_price = layers.fully_connected(self.rnn_price, label_len, activation_fn=None) #[batch*multi_count, 3]
        ...
        self.vec_mag_error = tf.reduce_mean(tf.square(self.convFlat2[:,-1,:] - self.rnn2_tn))
        self.vec_dist_error = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits_v2(labels=tf.nn.softmax(self.convFlat2[:,-1,:]), logits=self.rnn2_tn))

        l2_loss = tf.add_n([tf.nn.l2_loss(var) for var in tf.trainable_variables()])
        self.loss = tf.reduce_mean(self.td_error) + (self.lb_error+self.vec_mag_error+self.vec_dist_error) + (l2_loss * 1e-4)
...
class experience_buffer():
    ...
    def sample(self,batch_size):
        ...
        return np.reshape(sampledTraces,[batch_size,8])
...
def create_one_cell(scope,name):
    with tf.variable_scope(scope+'_LSTM',reuse=tf.AUTO_REUSE):
        cell = tf.contrib.rnn.LSTMCell(num_units=h_size, state_is_tuple=True, name=name, activation=tf.tanh)
        return cell

cell_fw1 = tf.contrib.rnn.MultiRNNCell([create_one_cell('cell_fw1',str(idx)) for idx in range(nb_lstm_layers)], state_is_tuple=True)
cell_bw1 = tf.contrib.rnn.MultiRNNCell([create_one_cell('cell_bw1',str(idx)) for idx in range(nb_lstm_layers)], state_is_tuple=True)
cell_fw2 = tf.contrib.rnn.MultiRNNCell([create_one_cell('cell_fw2',str(idx)) for idx in range(nb_lstm_layers)], state_is_tuple=True)
cell_bw2 = tf.contrib.rnn.MultiRNNCell([create_one_cell('cell_bw2',str(idx)) for idx in range(nb_lstm_layers)], state_is_tuple=True)

cellT_fw1 = tf.contrib.rnn.MultiRNNCell([create_one_cell('cellT_fw1',str(idx)) for idx in range(nb_lstm_layers)], state_is_tuple=True)
cellT_bw1 = tf.contrib.rnn.MultiRNNCell([create_one_cell('cellT_bw1',str(idx)) for idx in range(nb_lstm_layers)], state_is_tuple=True)
cellT_fw2 = tf.contrib.rnn.MultiRNNCell([create_one_cell('cellT_fw2',str(idx)) for idx in range(nb_lstm_layers)], state_is_tuple=True)
cellT_bw2 = tf.contrib.rnn.MultiRNNCell([create_one_cell('cellT_bw2',str(idx)) for idx in range(nb_lstm_layers)], state_is_tuple=True)

mainQN = Qnetwork(h_size,cell_fw1,cell_bw1,cell_fw2,cell_bw2,'main')
targetQN = Qnetwork(h_size,cellT_fw1,cellT_bw1,cellT_fw2,cellT_bw2,'target')
...
with tf.Session(config = gpuConfig) as sess:
    if load_model == True:
        ...
        if ckpt:
            ...
            e = endE
            anneling_steps = max_epLength * 10
            pre_train_steps = max_epLength * 10
            pre_train_switch = 'off'
    ...
    for i in range(num_episodes):
        ...
        while j < max_epLength: 
            ...
            if np.random.rand(1) < e or (total_steps < pre_train_steps and pre_train_switch == 'on'):
            ...
            total_steps += 1
            episodeBuffer.append(np.reshape(np.array([s,h,a,r,s1,h1,lb,lb1]),[1,8]))
            ...
            if total_steps > pre_train_steps:
                ...
                if total_steps % (update_freq) == 0:
                    ...
                    Q1 = sess.run(mainQN.predict,feed_dict={                        mainQN.scalarInput:np.vstack(trainBatch[:,4]),                        mainQN.hist_Input:np.vstack(trainBatch[:,5]),                        mainQN.batch_size:batch_size,                        mainQN.labels:np.vstack(trainBatch[:,7]),                        mainQN.training_flag:False})
                    Q2 = sess.run(targetQN.Qout,feed_dict={                        targetQN.scalarInput:np.vstack(trainBatch[:,4]),                        targetQN.hist_Input:np.vstack(trainBatch[:,5]),                        targetQN.batch_size:batch_size,                        targetQN.labels:np.vstack(trainBatch[:,7]),                        targetQN.training_flag:False})
                    doubleQ = Q2[range(batch_size*multi_count),Q1] #[batch*multi_count]
                    targetQ = np.vstack(trainBatch[:,3]).flatten() + (y*doubleQ)
                    
                    #Update the network with our target values.
                    sess.run(mainQN.updateModel,                         feed_dict={mainQN.scalarInput:np.vstack(trainBatch[:,0]),                                   mainQN.hist_Input:np.vstack(trainBatch[:,1]),                                   mainQN.targetQ:targetQ,                                   mainQN.actions:np.vstack(trainBatch[:,2]).flatten(),                                   mainQN.batch_size:batch_size,                                   mainQN.labels:np.vstack(trainBatch[:,6]),                                   mainQN.training_flag:True})
        ...
        if i % 1 == 0:
            print('[{}/{}][Total:{}/Pre:{}] Reward: {:.4f}, Score: {:.4f}'.format(i,num_episodes,total_steps,pre_train_steps,rAll,score))
